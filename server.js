...
async function piFetch(pathname, { method = "GET", network = "mainnet", accessToken, body } = {}) {
    ...
    if (!response.ok) {
        const err = new Error(data?.error_message || data?.message || `Pi API ${response.status}`);
...
// BONNE PRATIQUE : Remplacement de tous les caractères de saut de ligne (\n, \r)
// et des espaces multiples par un espace simple, pour éviter les erreurs
// d'arguments de commande avec espeak.
...
function buildSrt(segments) {
    ...
    segments.forEach((seg, i) => {
        srt += String(i + 1) + "\n"; // Correction : \n au lieu d'un saut de ligne littéral
...
/* ───────────── Helpers Traduction (locale) ───────────── */

async function translateText(text, sourceLang = "fr", targetLang = "en") {
    // Remplacer ultérieurement par un véritable moteur de traduction.
...
app.post("/api/pi/approve", async function (req, res) {
    try {
        ...
        if (paymentsCol) {
            await paymentsCol.updateOne({ paymentId: paymentId }, { $set: doc }, { upsert: true });
        } else {
...
app.post("/api/pi/complete", async function (req, res) {
    try {
        ...
        if (existing && existing.status === "completed") {
            ...
            return res.json({ ok: true, payment: existing, subscription: subscription, already: true });
        }
        ...
        if (paymentsCol) {
            await paymentsCol.updateOne({ paymentId: paymentId }, { $set: record }, { upsert: true });
        } else {
...
app.post("/api/create-subtitles", upload.single("video"), async function (req, res) {
    try {
        ...
        const translated = await translateSegments(whisperResult.segments, sourceLang, targetLang);
